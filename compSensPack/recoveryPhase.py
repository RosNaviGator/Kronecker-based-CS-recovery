import numpy as np
import sparseDictionaries as spD
import reconstructionMethods as recM
...
def non_kron_recovery(Y, sigma_min, Phi, sigma_decrease_factor=0.5, mu_0=2, L=3, showProgress=False):
    ...
    Dict = spD.generate_dct_dictionary(n_block)  # Dictionary
    ...
    for i in range(BLOCK_NUM):
        y = Y[:, i]
        s_block = recM.SL0(y, Theta, sigma_min, sigma_decrease_factor, mu_0, L, Theta_pinv, showProgress)
...
def kron_recovery(Y, sigma_min, Phi, kron_factor, sigma_decrease_factor=0.5, mu_0=2, L=3, showProgress=False):
    ...
    Dict_kron = spD.generate_dct_dictionary(n_block_kron)  # Dictionary (kron)
    ...
    for i in range(KRON_BLOCK_NUM):
        ...
        s_kron_block = recM.SL0(y_kron, Theta_kron, sigma_min, sigma_decrease_factor, mu_0, L, Theta_kron_pinv, showProgress)
